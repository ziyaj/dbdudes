...
\item Find the names of sailors who have reserved at least two different boats with the same color.
	\begin{itemize}
	\item \RA\\
	$rb := \pi_{sid, bid, color} (reserved \bowtie boats) $\\
	$good := \pi_{sid} (rb \bowtie_{rb.sid = rb2.sid\ \land \ rb.bid \neq rb2.bid\ \land \ rb.color = rb2.color} \rho_{rb2}(rb) )$\\
	$answer := \pi_{sname} (good \bowtie sailors)$
	\item \DL \\
...
\item Find the names of sailors who have reserved all red boats.
	\begin{itemize}
	\item \RA \\
	$ allred := \pi_{bid} (\sigma_{color = 'red'} boats)$ \\
	$ good := (\pi_{sid, bid} reserved) \div allred $\\
	$ answer := \pi_{sname}(good \bowtie sailors)$
	\item \DL \\
...
\item Find the name and color of boats which are reserved by all sailors rated above 7.
	\begin{itemize}
	\item \RA \\
	$ above7 := \pi_{sid}(\sigma_{rating > 7} sailors) $ \\
	$ witness := \pi_{sid, bid}(above7 \bowtie reserved) $ \\
	$ bad := \pi_{bid}(above7 \times \pi_{bid}reserved - witness) $ \\
	$ good := \pi_{bid}reserved - bad$ \\
	$ answer := \pi_{bname, color}(good \bowtie boats)$
	\item \DL \\
...
\item Find the name(s) of sailors with the lowest rating.
	\begin{itemize}
	\item \RA \\
	$ notlowest := \pi_{sailors.sid}(sailors \bowtie_{sailors.rating > sailors2.rating} \rho_{sailors2} (sailors)) $ \\
	$ lowest := \pi_{sname}((\pi_{sid}sailors - notlowest) \bowtie sailors) $
	\item \DL \\
...
\item Find the name and rating of the oldest sailor(s).
	\begin{itemize}
	\item \RA \\
	$ notoldest := \pi_{sailors.sid}(sailors \bowtie_{sailors.age < sailors2.age} \rho_{sailors2}(sailors))$ \\
	$ oldest := \pi_{sname, rating}((\pi_{sid} sailors - notoldest) \bowtie sailors) $
	\item \DL \\
...
\begin{itemize}
	\item \RA \\
	$ map := \rho_{sailors2.sid \to sid2}(sailors \bowtie_{rating > sailors2.rating} \rho_{sailors2} (sailors)) $ \\
	$ allcombo := \pi_{sid, sid2, bid}(map \bowtie_{map.sid2 = reserved.sid} reserved) $ \\
	$ witness := \pi_{sid, sid2, bid}(map \bowtie_{map.sid = reserved.sid} reserved)$ \\
	$ bad := \pi_{sid} (allcombo - witness) $ \\
	$ good := \pi_{sname}((\pi_{sid} reserved - bad) \bowtie sailors) $ 
	\item \DL \\
